...
def _build_report(cases: List[Dict[str, Any]]) -> Dict[str, Any]:
    return build_report(cases, format_name="reply_replay_eval")


def main() -> int:
    parser = argparse.ArgumentParser(description="运行自动回复 replay 回归")
    parser.add_argument("dataset", type=Path, help="replay JSON 数据集路径")
