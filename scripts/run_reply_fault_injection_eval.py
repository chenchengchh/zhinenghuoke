...
def main() -> int:
    parser = argparse.ArgumentParser(description="运行自动回复异常注入恢复评测")
    parser.add_argument("dataset", type=Path, help="异常注入 JSON 数据集路径")
    ...
    report = build_report(cases, format_name="reply_fault_injection_eval")
    ...
    report_path = output_dir / "reply_fault_injection_report.json"
    report_path.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

    print("=" * 72)
    print("Reply 异常注入评测完成")
